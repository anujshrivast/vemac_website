...
if ($batch_id > 0) {
    $batch_stmt = $conn->prepare("SELECT * FROM batches WHERE batch_id = ?");
    ...
    $formData = array_merge($formData, [
        'institute_name' => $batch_data['institute_name'],
    ...
    $teachers_stmt = $conn->prepare("SELECT teacher_id, subject FROM batch_teachers WHERE batch_id = ?");
    $teachers_stmt->bind_param("i", $batch_id);
    $teachers_stmt->execute();
    $teachers_result = $teachers_stmt->get_result();

    while ($teacher = $teachers_result->fetch_assoc()) {
        $formData['teacher_ids'][] = $teacher['teacher_id'];
        $formData['teacher_subjects'][] = $teacher['subject'];
    }

    // Fetch assigned students
    $students_stmt = $conn->prepare("SELECT student_id FROM batch_students WHERE batch_id = ? AND status = 'active'");
    $students_stmt->bind_param("i", $batch_id);
    $students_stmt->execute();
    $students_result = $students_stmt->get_result();

    while ($student = $students_result->fetch_assoc()) {
        $formData['students'][] = $student['student_id'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            // 1. Update batch information
            $stmt = $conn->prepare("UPDATE batches SET 
                institute_name = ?,
                batch_name = ?,
                batch_code = ?,
                course = ?,
            ...
            // 2. Remove existing teacher assignments
            $delete_teachers_stmt = $conn->prepare("DELETE FROM batch_teachers WHERE batch_id = ?");
            $delete_teachers_stmt->bind_param("i", $batch_id);
            $delete_teachers_stmt->execute();
            $delete_teachers_stmt->close();

            // 3. Insert updated teacher assignments with subjects
            ...
            // 4. Update student assignments - mark existing as inactive
            $inactive_stmt = $conn->prepare("UPDATE batch_students SET status = 'inactive', left_at = NOW() WHERE batch_id = ?");
            $inactive_stmt->bind_param("i", $batch_id);
            $inactive_stmt->execute();
            $inactive_stmt->close();

            // 5. Insert updated student assignments
            ...
            $errors['database'] = "Failed to update batch. Please try again. Error: " . $e->getMessage();
        }
    }
}
...
<head>
    ...
    <title><?= $batch_id > 0 ? 'Edit' : 'Create' ?> Batch</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    <style>
        /* Same CSS as in your original file */
        body {
...
<body>
    <div class="container">
        <h1><i class="fas fa-users"></i> <?= $batch_id > 0 ? 'Edit' : 'Create' ?> Batch</h1>
        ...
        <form id="batchForm" method="post" action="">
            ...
            <div class="form-actions">
                <a href="<?= $batch_id > 0 ? 'batch_details.php?id=' . $batch_id : 'batches.php' ?>" class="btn-secondary">
                    <i class="fas fa-arrow-left"></i> Cancel
                </a>
                <button type="submit" class="btn-primary">
                    <i class="fas fa-save"></i> <?= $batch_id > 0 ? 'Update' : 'Create' ?> Batch
                </button>
            </div>
        </form>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.removeTeacherBtn', function() {
                if ($('#teacherAssignments .teacher-assignment').length > 1) {
            ...
            $(document).on('click', '.removeStudentBtn', function() {
                if ($('#studentAssignments .student-assignment').length > 1) {
